    ...
    for index_set in tokens_index:
        ...
        if whole_word_masking:
            ...
            for j in range(mask_len):
                ...
                prob = random.random()
                if prob < 0.8:
                    src[i + j] = vocab.get(MASK_WORD_TOKEN)
                elif prob < 0.9:
                    while True:
                        rdi = random.randint(1, len(vocab) - 1)
                        if rdi not in [vocab.get(CLS_TOKEN), vocab.get(SEP_TOKEN), vocab.get(MASK_WORD_TOKEN), PAD_ID]:
                            break
                    src[i + j] = rdi
        elif span_masking:
            i = index_set[0]
            ...
            prob = random.random()
            if prob < 0.8:
                src[i] = vocab.get(MASK_WORD_TOKEN)
            elif prob < 0.9:
                while True:
                    rdi = random.randint(1, len(vocab) - 1)
                    if rdi not in [vocab.get(CLS_TOKEN), vocab.get(SEP_TOKEN), vocab.get(MASK_WORD_TOKEN), PAD_ID]:

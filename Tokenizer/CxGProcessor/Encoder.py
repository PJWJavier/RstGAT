...
class Encoder(object):
    def __init__(self, args=""):
        ...
        MODEL_STRING = os.path.abspath(os.path.join(os.path.dirname(__file__), "data/eng.RDR"))
        DICT_STRING = os.path.abspath(os.path.join(os.path.dirname(__file__), "data/eng.DICT"))
        DICTIONARY_FILE = os.path.abspath(os.path.join(os.path.dirname(__file__), "data/eng.clusters.fastText.v2.gz"))
...
if __name__ == "__main__":
    import sys
    sys.path.append("..")
    from constants import ARG_TEST
    args = ARG_TEST()
    encoder = Encoder()
    loader = Loader(args)
    text = ["The staff shouble be more a bit friendly.", "Are you ready?"]
    lines = loader.load_text(text)
    tagline = encoder.tagline(lines[0])
